...
function combatManuel(attaquant, attaque, idCoup) {
    ...
    frapperAnimation("perso1", "perso2", idCoup);
    afficheDegats(degats);
    affichageHistorique("MANUEL " + attaquant.nom + " a attaqué " + attaque.nom + " avec un " + attaqueNom[idCoup] + " et lui inflige " + degats + " dégats", "blue");
                        ...
                        var combattre = setInterval(function() {
                            if (perso2.isDead() || perso1.isDead()) {
                                ...
                                $(document).one("keydown", function(e) {
                                    if (typeof lastHitTime == 'undefined') {
                                        lastHitTime = 0;
                                    }
                                    if (lastHitTime + 500 < $.now()) {
                                        if (e.which == 65) {
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            for (i = 0; i < liste.length; i++) {
                ...
                $("#AllFights").append("<a href='http://localhost/" + window.location.pathname + "/#/fights/" + info[0] + "' onclick='location.reload()'><span>Combat: " + info[0] + " " + combattants[0][0] + " VS " + combattants[1][0] + "</span></a><br>")
            }
        },
    });
}
